...
import { Article, Browsers, CornersOut, Pause, Play, SpeakerSimpleHigh, SpeakerSimpleLow, SpeakerSimpleSlash } from 'phosphor-react'
...
const StyledTooltip = ({children, label, style}) => {
    return(
        <Tooltip style={style} sx={{height: 16}} color='dark' gutter={28} transition="pop" transitionDuration={150} transitionTimingFunction="ease"  label={<Text size='xs'>{label}</Text>}>
            {children}
        </Tooltip>
)}
    ...
    return (
        <Stack sx={{position: 'absolute', bottom: 0, width: '100%', height: '24px', padding: '0 16px 56px 16px'}}>
            ...
            <Group spacing='md' onMouseLeave={() => setVolumeOpen(false)}>
                ...
                <StyledTooltip style={{marginLeft: 'auto'}} label='Content Switcher'>
                    <Browsers onClick={() => setSelecting(true)} cursor='pointer' size={16} weight="fill" />
                </StyledTooltip>
                <StyledTooltip label='Full Screen'>
                    <CornersOut cursor='pointer' size={16} weight="bold" />
                </StyledTooltip>
            </Group>
            </Stack>
        </Stack>
